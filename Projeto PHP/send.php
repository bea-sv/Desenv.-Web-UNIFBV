...
if (mb_strlen($nome) > 120 || mb_strlen($descricao) > 5000) {
  http_response_code(413);
  exit('<p class="err">Conteúdo muito grande.</p>');
}

$nomeSafe     = htmlspecialchars($nome, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8');
$descricaoSafe = nl2br(htmlspecialchars($descricao, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8'));
...
try {
  ...
  $mail->Body    = "
    ...
    <p><strong>Mensagem:</strong><br>{$descricaoSafe}</p>
    <p><strong>Preço:</strong><br>{$preco}</p>
    <hr>
